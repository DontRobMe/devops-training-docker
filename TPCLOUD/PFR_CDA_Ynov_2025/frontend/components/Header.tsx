...
const Header: React.FC<HeaderProps> = ({ hasNotification }) => {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-40 bg-neutral-50 w-full flex justify-between items-center p-4">
            {/* Espace réservé à gauche, même largeur que le bouton */}
            <div className="w-10 h-10" /> {/* Simule l'espace de la cloche */}
            ...
            >
                ...
                {hasNotification && !isNotificationsPage && (
                    <span className="absolute top-[10px] right-[15px] block h-2.5 w-2.5 rounded-full bg-orange-500 transform translate-x-1/2 -translate-y-1/2"></span>
                )}
            </Button>
        </header>
    );
